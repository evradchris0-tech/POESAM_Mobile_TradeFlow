...
import 'transaction_documents_screen.dart';
...
class _EscrowScreenState extends State<EscrowScreen>
  ...
  void _confirmReceipt() async {
    ...
    // Loader pendant l'écriture Supabase
    showDialog(
      context: context,
    ...
    try {
      // rawId = UUID Supabase complet, disponible depuis fetchTransactions()
      // Si rawId est vide (mock data), on notifie l'utilisateur proprement
      final uuid = _tx.rawId.isNotEmpty ? _tx.rawId : null;
      if (uuid == null) throw Exception('UUID Supabase non disponible (mock data)');

      final updated = await TradeFlowApi.instance.confirmReceipt(uuid);

      if (!mounted) return;
      Navigator.pop(context); // ferme le loader

      setState(() {
        _tx = updated.copyWith(
        ...
        SnackBar(
          content: const Row(
            children: [
              ...
              SizedBox(width: 10),
              Expanded(
                child: Text('Réception confirmée. Trust Score mis à jour.'),
              ),
            ],
          ),
  ...
  void _reportIssue() {
    showModalBottomSheet(
      ...
      builder: (_) => _IssueSheet(transaction: _tx),
    );
  }
  ...
  Widget _quickActions(Transaction tx) {
    return Row(
      children: [
        Expanded(
          child: _quickActionButton(
            ...
            onTap: () => Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => TransactionChatScreen(transaction: tx),
              ),
            ),
          ),
        ),
        ...
        Expanded(
          child: _quickActionButton(
            ...
            onTap: () => Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => TransactionDocumentsScreen(transaction: tx),
              ),
            ),
          ),
        ),
          ...
          MaterialPageRoute(
            builder: (_) => ProductDetailScreen(
              product: ProductData(
                ...
                emoji: tx.emoji,
                shCode: tx.shCode,
                shDescription: tx.shCode.isNotEmpty ? tx.shCode : tx.product,
                unit: tx.unit,
                priceFcfa: tx.unitPrice,
                minOrderQty: tx.quantity > 0 ? tx.quantity : 1,
                availableStock: tx.quantity,
                description:
                    'Produit exporté via le corridor ${tx.corridor}.'
                    '${tx.transportRoute.isNotEmpty ? " Route : ${tx.transportRoute}." : ""}',
                origin: tx.sellerCity.isNotEmpty ? tx.sellerCity : 'Cameroun',
                corridor: tx.corridor,
                certifications:
                    tx.isTrustVerified ? ['Vendeur vérifié TradeFlow'] : [],
                vendorName: tx.sellerName,
                vendorCity: tx.sellerCity,
                vendorTrustScore: tx.sellerTrustScore,
              ),
            ),
          ),
        ),
            ...
            return Stack(
              ...
              children: [
                Container(
                  ...
                  height: 22 + 10 * t,
                  decoration: BoxDecoration(
                    color: orange.withValues(alpha: 0.35 * (1 - t)),
                    shape: BoxShape.circle,
                  ),
                ),
                Container(
...
class _IssueSheet extends StatefulWidget {
  ...
  const _IssueSheet({required this.transaction});
...
class _IssueSheetState extends State<_IssueSheet> {
  ...
  Future<void> _submit() async {
    ...
    final uuid = widget.transaction.rawId.isNotEmpty ? widget.transaction.rawId : null;
    if (uuid == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Impossible d\'ouvrir un litige sur une transaction de test.'), backgroundColor: danger),
      );
      return;
    }
    ...
    try {
      await TradeFlowApi.instance.openDispute(
        transactionId: uuid,
        reason: _selected!,
